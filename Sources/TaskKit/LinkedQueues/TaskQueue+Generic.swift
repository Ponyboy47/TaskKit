...
// 
// open class TypedTaskQueue<Type: Task> {
//     public typealias TaskType = Type
//     /// The name of the TaskQueue
//     public private(set) var name: String
// 
//     /// The tasks that are waiting to be run
//     public private(set) var waiting: [TaskType] = []
...
//     private var _beginning: [UUID: TaskType] = [:]
...
//     public private(set) var running: [UUID: TaskType] = [:]
...
//     public private(set) var _errored: [UUID: TaskType] = [:]
...
//     */
//     public convenience init(name: String, maxSimultaneous: Int = TaskQueue.defaultMaxSimultaneous, tasks: [TaskType]) {
...
//     */
//     public func addTask(_ task: TaskType) {
...
//     */
//     public func addTasks(_ tasks: [TaskType]) {
//         _waitingSemaphore.waitAndRun {
//             waiting += tasks
//             waiting.sort(by: { $0.priority.rawValue > $1.priority.rawValue })
//         }
...
//     - Parameter tasks: The tasks to add
//     */
//     public func add(tasks: [TaskType]) {
//         addTasks(tasks)
//     }
// 
//     /**
...
// 
//     /**
//     Prepares to execute the task by ensuring it is in the ready state and any dependencies are executed
// 
...
//     */
//     private func prepare(_ task: TaskType, with taskKey: UUID) -> TaskType? {
//         var task = task
//         switch task.state {
...
// 
//         guard var _task = task as? DependentTask else { return task }
// 
//         var dependency: TaskType? = nil
//         while !_task.dependencies.isEmpty {
//             let dep = _task.dependencies.removeFirst()
// 
//             let depKey = start(dep, autostart: false, dependent: _task)
//             _groups[depKey]!.wait()
// 
...
//         }
// 
//         guard _task.dependencies.isEmpty && dependency == nil else {
//             _task.state = .dependency(dependency!)
//             failed(_task as! TaskType, with: taskKey)
//             return nil
//         }
// 
//         _task.state = .done(.preparing)
//         return _task as? TaskType
//     }
// 
//     /**
...
//     - Returns: The task, if it was configured properly, or nil otherwise
//     */
//     private func configure(_ task: ConfigurableTask, with taskKey: UUID) -> TaskType? {
//         var task = task
//         task.state = .currently(.configuring)
//         guard task.configure() else {
//             failed(task as! TaskType, with: taskKey)
//             return nil
//         }
// 
...
//     */
//     private func execute(_ task: TaskType, with taskKey: UUID) -> Bool {
...
//             _runningSemaphore.waitAndRun {
//                 let task: TaskType! = running.removeValue(forKey: taskKey)
//                 failed(task, with: taskKey)
//             }
// 
//             return false
...
//     - Parameter taskKey: The unique key used to track the task
//     */
//     private func failed(_ task: TaskType, with taskKey: UUID) {
//         var task = task
...
//     */
//     @discardableResult
//     private func start(_ task: TaskType, autostart: Bool = true, dependent: DependentTask? = nil) -> UUID {
...
//                     let fail: TaskType! = running.removeValue(forKey: key)
...
//     */
//     @discardableResult
//     public func cancel() -> [TaskType] {
//         queue.suspend()
//         _isActive = false
// 
//         var cancelled: [TaskType] = []
//         _runningSemaphore.waitAndRun {
...
//     }
// 
//     private func cancelAllTasks() -> [TaskType] {
//         var cancelled: [TaskType] = []
//         for (key, _) in running {
//             var task: TaskType! = running.removeValue(forKey: key)
// 
//             // Look at stopping the dispatch queue/group for other ones (or both)
